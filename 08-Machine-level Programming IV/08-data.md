...
## 过程中的浮点
- `%xmm0~%xmm7`可以传递8个浮点参数，按照后面数字的顺序使用，多余的通过栈传递
- `%xmm0`存返回值
- XMM寄存器的保存由调用者完成
## 运算
![[Pasted image 20260616182132.png]]
## 浮点常数
![[Pasted image 20260616190748.png|590]]
这里使用了一种寻址（之前貌似没有遇见过）
每个标签对应的内存段是如此使用的：
```
3435973837 -> 0xcccccccd
1073532108 -> 0x3ffccccc
```
拼接得到（小端序）：`0x3ffccccc cccccccd`
然后按照浮点的位表示来解释，得到：`1.8`
## 比较
几乎和之前的一样
![[Pasted image 20260617141950.png]]
`S2`必须在XMM寄存器中
浮点比较会设置三个条件码：
- ZF
- CF
- PF（奇偶标志）：对于整数，当最近一次算术或逻辑运算产生的**值的最低位字节有偶数个1**，则设置该位。对于浮点数，两个操作数中任意一个为`NaN`时设置该位
![[Pasted image 20260617143542.png]]
（无序的意思就是任意操作数为`NaN`）

通常`jp`指令是条件跳转，条件就是**浮点比较得到一个无序的结果**。
