...
![[Pasted image 20260726201412.png|534]]
...
CPU使用**内存映射I/O**技术向I/O设备发射命令。使用内存映射I/O的系统中，*地址空间（所有地址可能取值构成的集合）* 中**有一块地址**是为与I/O设备通信保留的。每个这样的地址称为一个**I/O端口**
一个设备连接到总线时，它被映射到一个或多个端口（一个设备**可以发送不同种类的信息**）

1. CPU通过编写一个**三元组**——指令、逻辑块块号、内存地址，以启动一个读取操作
![[Pasted image 20260727171157.png|385]]
